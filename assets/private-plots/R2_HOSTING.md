...
Caveat: `deploy.yml`'s path filter also matches `**.html` unqualified
(not just paths outside `assets/private-plots/`), so committing a change
to the regenerated hub `index.html` itself can _also_ trigger the full
Jekyll build in parallel with the fast rsync path — this doesn't slow
anything down (the R2/rsync path still lands content live in ~1 minute
regardless of what else is running), it just means "no full rebuild" is
not a strict guarantee, only "content goes live fast regardless."
...
6. **Re-run the generator** so the hub page picks up the manifest change,
   ...
   If `<topic>/` (e.g. `hfss_sims/`) previously had _local_ galleries that
   have since all moved to R2, its intermediate hub file
   (`assets/private-plots/<topic>/index.html`, generated by
   `write_directory_hub_index`) goes stale: it still has relative links
   to the now-deleted local child directories, which the "Check for
   broken links" CI check will catch. The generator only creates hubs for
   directories that currently have local children, so it won't
   regenerate this file to fix itself — delete it by hand once every
   gallery under that topic is R2-only:

   ```bash
   git rm assets/private-plots/<topic>/index.html   # only if ALL its children are now R2-hosted
   ```
